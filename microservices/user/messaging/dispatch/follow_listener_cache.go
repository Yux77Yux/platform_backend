...
type FollowCacheListener struct {
	...
	next                *FollowCacheListener // 下一个监听者
	prev                *FollowCacheListener // 上一个监听者
}

func (listener *FollowCacheListener) GetId() int64 {
	return 0
}

// 启动监听者
func (listener *FollowCacheListener) StartListening() {
...
// 分发评论至通道
func (listener *FollowCacheListener) Dispatch(data protoreflect.ProtoMessage) {
	// 长度加1
...
// 执行批量更新
func (listener *FollowCacheListener) SendBatch() {
...
// 启动周期执行批量更新的定时器
func (listener *FollowCacheListener) RestartUpdateIntervalTimer() {
	// 先重置
...
// 启动存活时间的定时器
func (listener *FollowCacheListener) RestartTimeoutTimer() {
	// 先重置
...
// 清理监听者资源
func (listener *FollowCacheListener) Cleanup() {
	// 关闭评论通道
