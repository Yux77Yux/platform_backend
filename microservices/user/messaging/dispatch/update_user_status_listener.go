...
type UserStatusListener struct {
	...
	next                *UserStatusListener // 下一个监听者
	prev                *UserStatusListener // 上一个监听者
}

func (listener *UserStatusListener) GetId() int64 {
	return 0
}

// 启动监听者
func (listener *UserStatusListener) StartListening() {
	listener.userUpdateStatusChannel = make(chan *generated.UserUpdateStatus, LISTENER_CHANNEL_COUNT)
	go listener.RestartUpdateIntervalTimer()
	go listener.RestartTimeoutTimer()
}

// 分发至通道
func (listener *UserStatusListener) Dispatch(data protoreflect.ProtoMessage) {
	// 长度加1
...
// 执行批量更新
func (listener *UserStatusListener) SendBatch() {
...
// 启动周期执行批量更新的定时器
func (listener *UserStatusListener) RestartUpdateIntervalTimer() {
	// 先重置
...
// 启动存活时间的定时器
func (listener *UserStatusListener) RestartTimeoutTimer() {
	// 先重置
...
// 清理监听者资源
func (listener *UserStatusListener) Cleanup() {
	// 关闭通道
