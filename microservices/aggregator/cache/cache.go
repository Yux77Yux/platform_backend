...
func AddIpInSet(req *common.ViewCreation) error {
	...
	// 检查集合是否存在
	exists, err := CacheClient.Exists(context.Background(), "Ip_Creation")
	if err != nil {
		return err
	}
	...
	if !exists {
		// 集合不存在，插入数据并设置过期时间
		pipeline.SAdd(context.Background(), "Ip_Creation", idStr, ip)
		pipeline.Expire(context.Background(), "Ip_Creation", 5*time.Minute)
	} else {
		// 集合已存在，只插入数据
		pipeline.SAdd(context.Background(), "Ip_Creation", idStr, ip)
	}
