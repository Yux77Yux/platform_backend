...
func ServerRun(ctx context.Context) {
	...
	case <-time.After(time.Minute):
		grpcServer.Stop()
		tools.LogWarning(traceId, "Server Run", "ServerRun timeout reached. Forcing shutdown")
	}
}

type Server struct {
	generated.UnimplementedAggregatorServiceServer
}
...
func InitServer(grpcServer *grpc.Server) {
	...
	generated.RegisterAggregatorServiceServer(grpcServer, &Server{}) // 注册 User 服务
