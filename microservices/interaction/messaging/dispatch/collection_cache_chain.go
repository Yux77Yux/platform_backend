...
// 责任链
type CollectionCacheChain struct {
	Head         *CollectionListener // 责任链的头部
...
func (chain *CollectionCacheChain) Close(signal chan any) {
...
func (chain *CollectionCacheChain) GetPoolObj() any {
...
// 处理评论请求的函数
func (chain *CollectionCacheChain) HandleRequest(data protoreflect.ProtoMessage) {
...
// 查找责任链中的合适监听者
func (chain *CollectionCacheChain) FindListener(data protoreflect.ProtoMessage) ListenerInterface {
	chain.nodeMux.Lock()
...
// 创建一个新的监听者
func (chain *CollectionCacheChain) CreateListener(data protoreflect.ProtoMessage) ListenerInterface {
	newListener := chain.listenerPool.Get().(*CollectionListener)
...
// 销毁监听者
func (chain *CollectionCacheChain) DestroyListener(listener ListenerInterface) {
	// 找到前一个节点（假设 chain.Head 是链表的头部）
	current, ok := listener.(*CollectionListener)
	if !ok {
		log.Printf("invalid type: expected *CollectionListener")
	}
