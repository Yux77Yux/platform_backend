...
type CancelLikeListener struct {
	...
	next                *CancelLikeListener // 下一个监听者
	prev                *CancelLikeListener // 上一个监听者
}

func (listener *CancelLikeListener) GetId() int64 {
	return 0
}
...
// 分发评论至通道
func (listener *CancelLikeListener) Dispatch(data protoreflect.ProtoMessage) {
	// 长度加1
...
// 执行批量更新
func (listener *CancelLikeListener) SendBatch() {
...
// 启动存活时间的定时器
func (listener *CancelLikeListener) RestartTimeoutTimer() {
	// 先重置
...
// 清理监听者资源
func (listener *CancelLikeListener) Cleanup() {
	// 关闭评论通道
