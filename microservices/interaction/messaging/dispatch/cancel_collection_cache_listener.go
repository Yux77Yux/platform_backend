...
// 监听者结构体
type CancelCollectionListener struct {
	...
	next                *CancelCollectionListener // 下一个监听者
	prev                *CancelCollectionListener // 上一个监听者
}

func (listener *CancelCollectionListener) GetId() int64 {
	return 0
}

// 启动监听者
func (listener *CancelCollectionListener) StartListening() {
	listener.RestartUpdateIntervalTimer()
	listener.RestartTimeoutTimer()
}

// 分发评论至通道
func (listener *CancelCollectionListener) Dispatch(data protoreflect.ProtoMessage) {
	// 长度加1
...
// 执行批量更新
func (listener *CancelCollectionListener) SendBatch() {
...
// 启动周期执行批量更新的定时器
func (listener *CancelCollectionListener) RestartUpdateIntervalTimer() {
...
// 启动存活时间的定时器
func (listener *CancelCollectionListener) RestartTimeoutTimer() {
	// 先重置
	...
	listener.timeoutTimer = time.AfterFunc(listener.timeoutDuration, func() {
		...
		if count == 0 {
			// 超时后销毁监听者
			listener.Cleanup()
			cancelCollectionCacheChain.DestroyListener(listener)
		} else {
...
// 清理监听者资源
func (listener *CancelCollectionListener) Cleanup() {
	// 关闭评论通道
